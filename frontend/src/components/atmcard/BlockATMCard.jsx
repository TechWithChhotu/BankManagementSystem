...
function IssueAtmCard() {
  ...
  const handleSetPin = async (e) => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:3000/api/v1/system/block-atm-card",
        formData,
  ...
  return (
    <div>
      ...
      >
        <CiCreditCardOff className="w-40 h-40" />
        <span className="text-[10px]">Block ATM Card</span>
      </button>
      {isOpen && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
          <div className="bg-white p-8 rounded-lg shadow-lg w-full max-w-md ">
            <div className="w-full flex flex-col items-center justify-center mb-4 ">
              ...
              <h2 className="text-2xl font-bold mb-4">Block ATM Card</h2>
            </div>
            <form>
              {isYouEmployee && (
                <div className="mb-4">
